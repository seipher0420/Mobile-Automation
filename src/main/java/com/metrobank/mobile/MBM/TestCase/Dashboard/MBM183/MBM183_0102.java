...
public class MBM183_0102 extends MobileConnection{
	...
	public void DashBoardViewDepositAccountOnly0102() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("MBM 183 Allow customer to select enroll on my accounts 0102");
		...
		try{
			...
			logGeneration.inputLogs(LogType.pass, "Input Account Number is displayed", null);
		}catch(AssertionError e){
		...
		try{
			...
			logGeneration.inputLogs(LogType.pass, "Input Account Alias is displayed", null);
		}catch(AssertionError e){
		...
		TestUtil.waitTime(5);
		try{
			Assert.assertFalse(enrollmentPage.enrollmentBtnIsEnabled(driver));
			logGeneration.inputLogs(LogType.pass, "Enroll Button is Disabled", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Enroll Button is enabled", TestUtil.getScreenshot(driver));
		}

		try{
			Assert.assertFalse(enrollmentPage.cancelButtonIsDisplayed(driver));
			logGeneration.inputLogs(LogType.pass, "Cancel Button is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Cancel Button is not displayed", TestUtil.getScreenshot(driver));
		}
