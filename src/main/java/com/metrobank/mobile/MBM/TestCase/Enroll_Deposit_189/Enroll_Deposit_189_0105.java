...
/**
 * @Description
 * Login Enroll Deposit_189 01.05 
 * Enroll account screen when alias already assigned to other added or enrolled accounts
 * @author testuser
 *
 */

public class Enroll_Deposit_189_0105 extends MobileConnection {
	...
	public void connectionMobile() throws InterruptedException, IOException{
		...
		String existingAlias = property.GetValue("existingAlias");
		
		// Setup Logger
		LogGeneration logGeneration = new LogGeneration();
		logGeneration.generateReport("Enroll Deposit_189 01.05");
		...
		myAccountsPage.inputAccountNumber(driver, accountNumber);
			...
			TestUtil.waitTime(1);
		myAccountsPage.inputAccountAlias(driver, existingAlias);
			logGeneration.inputLogs(LogType.info, 
				String.format("Input Account Alias : {%s}", existingAlias),  null);
			TestUtil.waitTime(1);
		...
		if (myAccountsPage.isErrorAliasAlreadyInUseDisplayed(driver) == true) {
