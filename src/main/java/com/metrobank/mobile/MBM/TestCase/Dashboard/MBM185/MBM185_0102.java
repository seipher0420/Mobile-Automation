...
public class MBM185_0102 extends MobileConnection{
	...
	public void DashBoardViewDepositAccountOnly0102() throws IOException, InterruptedException{
		...
		try{
			Assert.assertTrue(enrollmentPage.enrollmentVerifyOtpPageIsDisplayed(driver));
			logGeneration.inputLogs(LogType.pass, "Verify OTP Page is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Verify OTP Page is not Displayed", TestUtil.getScreenshot(driver));
			
		}
		TestUtil.waitTime(3);
		try{
			Assert.assertTrue(enrollmentPage.enrollmentVerifyOtpPageNextBtnIsEnabled(driver));
			logGeneration.inputLogs(LogType.pass, "Next Button is disabled", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Next Button is Enabled", TestUtil.getScreenshot(driver));
