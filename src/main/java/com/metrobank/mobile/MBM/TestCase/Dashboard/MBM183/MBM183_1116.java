...
public class MBM183_1116 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);

	@Test
	public void DashBoardViewDepositAccountOnly0116() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("MBM 183 Enter an account number that is not owned by the logged in user 1116");
		...
		String accountNumber = property.GetValue("AccountNumber_183116");
		String alias = property.GetValue("AccountAlias_183116");
		...
		try{
			Assert.assertTrue(enrollmentPage.enrollmentDepositAccountNotExistErroredDisplayed(driver));
			logGeneration.inputLogs(LogType.pass, "Error Message Account does not exist is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Error Message Account does not exist is not displayed", TestUtil.getScreenshot(driver));
		}
