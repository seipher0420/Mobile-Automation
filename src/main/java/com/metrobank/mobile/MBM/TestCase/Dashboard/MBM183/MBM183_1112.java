...
public class MBM183_1112 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);

	@Test
	public void DashBoardViewDepositAccountOnly0112() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("MBM 183 Validation of required field with invalid inputted data 0112");
		...
		String accountNumber = property.GetValue("AccountNumber_183112");
		String alias = property.GetValue("AccountAlias_183112");
		...
		try{
			Assert.assertTrue(enrollmentPage.enrollAccountErrorInvalidInput(driver) != null);
			logGeneration.inputLogs(LogType.pass, "Error Invalid input. Enter the number in the correct format. is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "No Error was displayed", TestUtil.getScreenshot(driver));
		}
		TestUtil.waitTime(3);
		try{
			Assert.assertFalse(enrollmentPage.enrollAccountAliasIsVerified(driver));
			logGeneration.inputLogs(LogType.pass, "Error Invalid input. Enter the number in the correct format. is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "No Error was displayed", TestUtil.getScreenshot(driver));
		}
