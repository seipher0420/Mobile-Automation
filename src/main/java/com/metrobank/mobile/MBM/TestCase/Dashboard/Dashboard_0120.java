...
public class Dashboard_0120 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);
	
	@Test
	public void ValidationLogOutBtnNavigationBar0120() throws IOException, InterruptedException{
		LogGeneration logGeneration = new LogGeneration();
		LoginPage loginPage = new LoginPage();
		DashBoard dashboard = new DashBoard();
		
		logGeneration.generateReport("0120 ValidationLogOutBtn on Navigation Bar");
		...
		try{
			...
			try{
				...
				logGeneration.inputLogs(LogType.pass, "DashBoard Menu is Displayed upon clicking Dashboard choice on Hamburger Menu", null);
		...
		try{
			...
			try{
				TestUtil.waitTime(2);
				Assert.assertTrue(dashboard.hamburgerMyAccountMenuIsDisplayed(driver));
				logGeneration.inputLogs(LogType.pass, "My Accounts Page is displayed", null);
			}catch(AssertionError e){
		...
		try{
			...
			logGeneration.inputLogs(LogType.pass, "My Accounts Deposit Menu is displayed", null);
		}catch(AssertionError e){
		...
		try{
			...
			logGeneration.inputLogs(LogType.pass, "My Accounts Credit Menu is displayed", null);
		}catch(AssertionError e){
		...
		try{
			Assert.assertTrue(dashboard.myAccountDepositMenuIsDisplayed(driver));
			logGeneration.inputLogs(LogType.pass, "My Accounts Prepaid Menu is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "My Accounts Prepaid Menu is not displayed", TestUtil.getScreenshot(driver));
		}
		TestUtil.waitTime(3);
		dashboard.clickhamburgerMenuMyAccounts(driver);
		logGeneration.inputLogs(LogType.info, "Click Hamburger Menu", null);
		...
		logGeneration.inputLogs(LogType.info, "Click LogOut Button in Hamburger Menu", null);
		TestUtil.waitTime(3);
		dashboard.clickLogoutbtnMenu(driver);
		logGeneration.inputLogs(LogType.info, "Click LogOut Button in Logout dialog", null);
