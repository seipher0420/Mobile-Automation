...
public class MBM183_1114 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);

	@Test
	public void DashBoardViewDepositAccountOnly0114() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("MBM 183 Enter existing account number 0114");
		...
		String accountNumber = property.GetValue("AccountNumber_183114");
		String alias = property.GetValue("AccountAlias_183114");
		...
		try{
			Assert.assertTrue(enrollmentPage.enrollmentDepositErrorEnrolledDisplayed(driver));
			logGeneration.inputLogs(LogType.pass, "Error Message Deposit Account Already Enrolled is displayed", null);
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Error Message Deposit Account Already enrolled is not displayed", TestUtil.getScreenshot(driver));
		}
