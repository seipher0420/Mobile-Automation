...
public class DepositAccountOverview_0112 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);

	@Test
	public void DepositAccountOverview0112() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("Transction History using Keyword Filter Without Transaction 0112");
		...
		dashboard.inputDepositOverviewSearch(driver, "Test");
		logGeneration.inputLogs(LogType.info, "Input Search Value", null);
		TestUtil.waitTime(3);
		
		try{
			Assert.assertTrue(dashboard.getDepositOverviewSearchMessage(driver) != null);
			logGeneration.inputLogs(LogType.pass, "System Displayed message: " + dashboard.getMessageNoTransaction(driver), TestUtil.getScreenshot(driver));
		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "System didn't display message", TestUtil.getScreenshot(driver));
		}
