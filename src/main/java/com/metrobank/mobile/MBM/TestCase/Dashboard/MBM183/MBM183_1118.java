...
public class MBM183_1118 extends MobileConnection{

	PropertyFileHandler property = new PropertyFileHandler(Locators.TESTDATA);

	@Test
	public void DashBoardViewDepositAccountOnly0118() throws IOException, InterruptedException{
		...
		logGeneration.generateReport("MBM 183 validation of the Account Alias field after user has entered a long alias 0118");
		...
		String accountNumber = property.GetValue("AccountNumber_183118");
		String alias = property.GetValue("AccountAlias_183118");
		...
		try{
			Assert.assertTrue(enrollmentPage.enrollAccountAliasIsVerified(driver));
			logGeneration.inputLogs(LogType.pass, "No Error was displayed", null);

		}catch(AssertionError e){
			logGeneration.inputLogs(LogType.fail, "Error Invalid input. Enter the number in the correct format. is displayed", TestUtil.getScreenshot(driver));
		}
